...
class MentorizeController(http.Controller):
    ...
    # ALUMNI PROFIL
    # =====================
    @http.route('/mentorize/alumni/profil', auth='user', website=True)
    def profil_alumni(self, **kw):
        user = request.env.user
        alumni = request.env['mentorize.alumni'].sudo().search([
            ('user_id', '=', user.id)
        ], limit=1)

        return request.render('mentorize.page_profil_alumni', {
            'user': user,
            'alumni': alumni,
        })


    @http.route('/mentorize/alumni/profil/edit', auth='user', website=True)
    def edit_profil_alumni(self, **kw):
        user = request.env.user
        alumni = request.env['mentorize.alumni'].sudo().search([
            ('user_id', '=', user.id)
        ], limit=1)

        return request.render('mentorize.page_edit_profil_alumni', {
            'user': user,
            'alumni': alumni,
        })


    @http.route('/mentorize/alumni/profil/update', auth='user', website=True, methods=['POST'])
    def update_profil_alumni(self, **post):

        user = request.env.user
        Alumni = request.env['mentorize.alumni'].sudo()

        alumni = Alumni.search([('user_id', '=', user.id)], limit=1)

        vals = {
            'company': post.get('company'),
            'posisi': post.get('posisi'),
            'bidang': post.get('bidang'),
            'linkedin': post.get('linkedin'),
            'bio': post.get('bio'),
            'pengalaman': post.get('pengalaman'),
            'slot': post.get('slot'),
        ...
        return request.redirect('/mentorize/alumni/profil')
